...
import { Video, Cpu, FileText, Layers, ListOrdered, CheckCircle2, AlertCircle, Wrench } from 'lucide-react';
    ...
    <header className="bg-[#121316]/90 border-b border-zinc-800/80 sticky top-0 z-30 shadow-md backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          {/* System CLI Tools Status Badges */}
          <div className="flex items-center gap-3">
            <button
              onClick={onOpenDiagnostics}
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-zinc-900/80 hover:bg-zinc-800/90 border border-zinc-700/70 text-zinc-200 transition-colors shadow-xs"
              title="查看 yt-dlp, ffmpeg, whisper-cli 运行环境诊断"
            >
              <Wrench className="w-3.5 h-3.5 text-zinc-400" />
              <span className="hidden md:inline text-zinc-300">环境诊断:</span>
